...
from tests.test_funcs.TestFunc2D import func_fd_2d_cl2_c1_06
import tools.ncalls as ncalls


def main(log_level):
    my_settings = FaultApproxParameters()
    my_settings.max_dist_for_surface_points = 0.02
    my_settings.abstol_bisection = 0.001
    my_settings.num_points_local = 7
    my_settings.err_max = 0.002
    my_settings.err_min = 1e-4

    my_prob = ProblemDescr()
    my_prob.output_file_vtu = "testFaultDetection2D_14.vtu"
    my_prob.test_func = func_fd_2d_cl2_c1_06

    my_prob.x_min = np.array([0, 0])
    my_prob.x_max = np.array([1, 1])

    my_prob.verbose_mode = True
